...
export default class Storage implements ProxyStorage {
  ...
  static clearAll(nameSpace = "responsive", options: object) {
  ...
  // 获取key对应的value
  static getData(nameSpace = "responsive", key: string) {
    const lsKey = `${nameSpace}-${key}`;
    if (Object.prototype.hasOwnProperty.call(window.localStorage, lsKey)) {
  ...
  public constructor(app: App<Element>, nameSpace = "responsive", options: any = {}) {
    const self = this;
